...
export default class itemList extends Component {
    ...
    render() {
        const addItem = itemInfo => {
            ...
            /*Axios.put('http://192.168.1.175:3000',JSON.stringify(obj),config).then(res => {return res.json()}).catch(err =>{console.log(err)})*/
            fetch(`http://192.168.1.175:3000/items/${id}`, {
                    method: 'put',
                    headers: {
        ...
        const handleSummit = () => {
            ...
            addItem(itemInfo).then(item => {
                if(item){
                    ...
                    Alert.alert('Item has been changed :p','your modified item is up and running',[{text: 'Ok'}])
                }
                else {
